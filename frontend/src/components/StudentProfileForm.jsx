...
import { extractResume } from "../api/extractionAPI";
...
export default function StudentProfileForm() {
  ...
  const validateForm = () => {
    ...
    if (!formData.branch) return alert("Please select Branch!") || false;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Step 2: Save student profile to backend
      const payload = {
        first_name: formData.first_name,
        last_name: formData.last_name,
        roll_no: formData.roll_no,
      ...
      // Step 3: Send resume PDF for AI extraction
      try {
        await extractResume(resumeFile);
      } catch (aiErr) {
        console.warn("AI extraction failed (profile still saved):", aiErr);
      }
